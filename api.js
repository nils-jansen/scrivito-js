...
class Scrivito {
  ...
  async getObject(id) {
    return new Promise(async (resolve, reject) => {
      try {
        let response = await axios.get(`https://api.scrivito.com/tenants/${this.#tenantID}/workspaces/${this.#workspaceID}/objs/${id}`,undefined,{
          auth: {
            username: "api_token",
  ...
  async deleteObject(id) {
    return new Promise(async (resolve, reject) => {
      try {
        let response = axios.delete(`https://api.scrivito.com/tenants/${this.#tenantID}/workspaces/${this.#workspaceID}/objs/${id}`, undefined, {
          auth: {
            username: "api_token",
            password: this.#apiKey
          }
        })
        if (Math.floor(response.status / 100) == 2) { // TODO: Test if success response is actually 2xx code
          resolve();
        } else {
          reject(response);
        }
  ...
  async updateObject(obj){
    return new Promise( async (resolve, reject) => {
      if(obj && obj._id){
        try {
          let response = await axios.post(`https://api.scrivito.com/tenants/${this.#tenantID}/workspaces/${this.#workspaceID}/objs`, obj,
          {
            auth: {
              username: "api_token",
              password: this.#apiKey
  ...
  async createWorkspace(title){
    return new Promise( async (resolve, reject) => {
      if(title){
        try {
          let response = await axios.post(`https://api.scrivito.com/tenants/${this.#tenantID}/workspaces`,{
    ...
    return new Promise( async (resolve, reject) => {
      if(id){
        try {
          ...
          `https://api.scrivito.com/tenants/${this.#tenantID}/workspaces/${id}/publish`,
          undefined,
          {
            auth: {
              username: "api_token",
